...
public class ECBMDFetcher implements MDFetcher {
    ...
    public boolean fetch() {
        try {
            EcbClient.api()
                    ...
                    .map(v -> v.stream().map(exchangeRate -> new com.batiaev.md.keepr.model.ExchangeRate(
    ...
    public boolean fetch(LocalDate date) {
        try {
            return process(Map.of(date, EcbClient.api().rates(date)));
        } catch (Exception e) {
            e.printStackTrace();
            return false;
        }
    ...
    private boolean process(Map<LocalDate, List<ExchangeRate>> rates) {
        try {
            rates
                    .entrySet()
                    .stream()
